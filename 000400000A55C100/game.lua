...
function updateGame(dt)
	...
	if game_state == "START" then
		...
		--love.system.getModel() == "2ds"
		
		--if ((mouse_x >= 219) and (mouse_x <= 219 + 100)) and ((mouse_y >= 0) and (mouse_y <= 0 + 69)) then
		if (love.keyboard.isDown("x") and love.keyboard.isDown("y")) then
		
			--Here
			guy_slide = guy_slide + 1
		...
		if love.keyboard.isDown(left_btn) then
			...
			level_x = math.min(0, level_x + (guy_speed * 4 * dt) * 60)
		...
		if (noPills() == false) or (guy_slide > 120) then
...
drawTopScreen()
	...
	if game_state == "TITLE_SCREEN" then
		...
		if dsp_dumped == false then
			love.graphics.setColor(c_white)
			love.graphics.rectangle("fill", 0, 0, 400, 48)
			love.graphics.setColor(c_black)
			love.graphics.print("Your DSP has not been dumped!", 4, 4) --If you want audio to work, please visit: http://tinyurl.com/dspdump
			love.graphics.print("To enable audio, please visit: http://tinyurl.com/dspdump", 4, 24)
			love.graphics.setColor(c_white)
		end
	end
